...
public class Xls_Reader {
	...
	public String getNames(String sheetName, int colNumber) {
		int index = workbook.getSheetIndex(sheetName);
		sheet = workbook.getSheetAt(index);
		int rowCount = sheet.getLastRowNum();
		System.out.println("Row count: " + rowCount);
		for(int i = 1; i < rowCount+1; i++) {
			return sheet.getRow(i).getCell(colNumber).getStringCellValue();
		}		
		return sheetName ;
	}
	
	//Get random names from excel sheet
	@SuppressWarnings("rawtypes")
	public String getCellData(String sheetName) throws InterruptedException {
		int index = workbook.getSheetIndex(sheetName);
        sheet = workbook.getSheetAt(index);
        int randomRowIndex = new SecureRandom().nextInt(sheet.getLastRowNum()) % 
